  ...
  return (
    <div className="px-3 py-2.5 border-t border-border/20">
      <div className="flex flex-wrap items-stretch gap-2 max-w-3xl mx-auto">
        ...
        <button onClick={onToggleCamera} title={cameraOn ? "Turn off camera" : "Turn on camera"}
          className={`h-12 w-12 rounded-xl flex items-center justify-center transition-all duration-200 shadow-sm
            ${!cameraOn ? "bg-destructive/20 text-destructive shadow-[0_2px_10px_hsl(var(--destructive)/0.2)]" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
          {cameraOn ? <Video className="w-4.5 h-4.5" /> : <VideoOff className="w-4.5 h-4.5" />}
        </button>

        <button onClick={onToggleMic} title={micOn ? "Mute" : "Unmute"}
          className={`h-12 w-12 rounded-xl flex items-center justify-center transition-all duration-200 shadow-sm
            ${!micOn ? "bg-destructive/20 text-destructive shadow-[0_2px_10px_hsl(var(--destructive)/0.2)]" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"}`}>
        ...
        <div className="relative flex-1 min-w-[120px]" ref={dropdownRef}>
          <button onClick={() => setDropdownOpen((o) => !o)}
            className="w-full h-12 rounded-xl bg-secondary text-secondary-foreground flex items-center justify-between gap-2 px-3.5 shadow-sm hover:bg-secondary/80 transition-all duration-200">
          ...
          {dropdownOpen && (
            <div className="absolute bottom-full mb-1 left-0 w-full max-h-48 overflow-y-auto rounded-xl bg-card border border-border shadow-lg z-50 py-1 animate-fade-in">
              {COUNTRIES.map((c) => (
                <button key={c.code} onClick={() => { onCountryChange(c); setDropdownOpen(false); }}
                  className={`w-full flex items-center gap-2.5 px-3.5 py-2 text-sm hover:bg-muted/50 transition-colors ${c.code === country.code ? "bg-primary/10 text-primary" : "text-foreground"}`}>
        ...
        <div className="h-12 rounded-xl bg-secondary shadow-sm flex items-center p-1 gap-0.5">
          <button onClick={() => onGenderChange("boy")}
            className={`h-full px-3.5 rounded-lg text-sm font-medium flex items-center gap-1.5 transition-all duration-300
              ${gender === "boy" ? "bg-primary/15 text-primary shadow-[0_0_10px_hsl(var(--primary)/0.2)]" : "text-muted-foreground hover:text-foreground"}`}>
            <span className="text-base">👦</span>
            <span className="hidden sm:inline text-xs">I am</span>
          </button>
          <button onClick={() => onGenderChange("girl")}
            className={`h-full px-3.5 rounded-lg text-sm font-medium flex items-center gap-1.5 transition-all duration-300
              ${gender === "girl" ? "bg-[hsl(330_70%_50%/0.15)] text-[hsl(330_70%_55%)] shadow-[0_0_10px_hsl(330_70%_50%/0.2)]" : "text-muted-foreground hover:text-foreground"}`}>
            <span className="text-base">👧</span>
            <span className="hidden sm:inline text-xs">I am</span>
          </button>
        </div>
